...
//            ReporterStatus += " - "+ e.getMessage();
        ...
        try {
            sessionID = baseTest.sessionID;
            testName = context.getDisplayName();

            deviceSN = baseTest.device.getSerialnumber();
            agent = baseTest.device.getAgent();
            deviceOS_isIOS = baseTest.device.isIOS();
            isEspresso_XCUI = baseTest.isEspresso_XCUI;


            reportPath = baseTest.reportURL;
            if (!reportPath.contains("http") && !reportPath.contains("ReportURL")) {
                reportPath = reportPath + "/index.html";
            }
            EndTime = String.valueOf(baseTest.testEndTime);
            StartTime = String.valueOf(baseTest.testStartTime);
            testDuring = System.currentTimeMillis() - baseTest.testStartTime_calculate;
...
//            System.out.println("the test passed");
    ...
    protected String getDeviceOSinString() {
        String DeviceOSinString="";
        if (deviceOS_isIOS) {
            DeviceOSinString = "IOS";
        } else {
            DeviceOSinString ="Android";
        }

        if(isEspresso_XCUI){
            DeviceOSinString += " - Espresso_XCUI";
        }

        return DeviceOSinString;
    }
